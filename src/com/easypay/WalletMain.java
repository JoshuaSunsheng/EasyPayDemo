...
public class WalletMain {
    ...
    //查询账户信息
    public static void getWalletInfo(long userId){
        JSONObject sParaTemp = new JSONObject();
        sParaTemp.put("merchant_id", merchant_id);
        sParaTemp.put("user_id", userId);

        biz_content = sParaTemp.toString();
        service  = "easypay.wallet.info";
    }
    //查询账户交易明细
    public static void getWalletTradeList(long userId){
        JSONObject sParaTemp = new JSONObject();
        sParaTemp.put("merchant_id", merchant_id);
        sParaTemp.put("user_id", userId);
        sParaTemp.put("start_date", "2020-01-17");
        sParaTemp.put("end_date", "2020-01-17");
        sParaTemp.put("start", 0);
        sParaTemp.put("page_size", 10);
        sParaTemp.put("trade_type", 0);

        biz_content = sParaTemp.toString();
        service  = "easypay.wallet.tradeList";
    }

    //新无卡-协议支付-支付
    public static void consume(long userId, String card){
        JSONObject sParaTemp = new JSONObject();
        sParaTemp.put("merchant_id", merchant_id);
        sParaTemp.put("user_id", userId);
        ...
        sParaTemp.put("password", "11111166");
        sParaTemp.put("pay_type", 1);//1 钱包 2 银行卡
        sParaTemp.put("card_no", 1);//1 钱包 2 银行卡

        biz_content = sParaTemp.toString();
        service  = "easypay.wallet.consume";
    }
    //订单查询
    public static void orderQuery(long userId){
        JSONObject sParaTemp = new JSONObject();
        sParaTemp.put("merchant_id", merchant_id);
        sParaTemp.put("user_id", userId);
        sParaTemp.put("out_trade_no", "114591_1573558276000");
        sParaTemp.put("trade_type", 504);

        biz_content = sParaTemp.toString();
        service  = "easypay.wallet.order.query";
    }

    //新无卡-协议支付-账户认证
    public static void validateAccount(long userId){
        JSONObject sParaTemp = new JSONObject();
        sParaTemp.put("merchant_id", merchant_id);
        sParaTemp.put("user_id", userId);
        sParaTemp.put("name", getEncode("全渠道"));    //账户姓名
        sParaTemp.put("id_no", getEncode("100030033330030044")); //身份证号
        sParaTemp.put("card_no", getEncode("6226188887788788"));   //银行卡号
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            //账户充值
//            WalletMain.consume("ES2019041800105873", 8);
            //账户查询
            WalletMain.getWalletInfo(116194);
            //账户明细查询
//            WalletMain.getWalletTradeList(100701);
//            WalletMain.orderQuery(100701);
//            WalletMain.consume(100701);
//            新无卡-直接支付
